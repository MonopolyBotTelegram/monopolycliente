async function DepositAlertaUpdate_m(depositAlerta) {
    let intentos = 0;
    while (intentos < 10) {
        try {
...
const resultado = await new Promise((resolve, reject) => {
                ...
                socket.emit("DepositAlertaUpdate", {
                    userId: VARIABLES_get_chatId(),
                    password:VARIABLES_get_password(),
                    depositAlerta:depositAlerta
                });

                // Escuchar la respuesta del servidor
                socket.once("DepositAlertaUpdateResponse", (data) => {
                    if (data.error) {
